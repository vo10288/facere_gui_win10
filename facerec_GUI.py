...
def openImagesx():
	...
	if not os.path.exists("reconunknown"):
		os.makedirs("reconunknown")
	if not os.path.exists("reconknown"):
		os.makedirs("reconknown")

	global imagesx
	imagesx = tkinter.filedialog.asksaveasfilename(initialdir = ".",title = "Select file",filetypes = (("all files","*.jpg"),("all files","*.png"),("all files","*.jpeg")))
	print (imagesx)
	
	global imagesxGIF
	#imagesxGIF = = datetime.now().strftime("%Y_%m_%d_%H_%M_%S") +'.gif'
	imagesxGIF = 'known.gif'
	print(imagesxGIF)
	
	command = ('convert '+ imagesx + ' -resize 300x200 gif:'+'reconknown\\'+imagesxGIF)
	subprocess.Popen(command, shell=True)
	print('reconknown\\'+imagesxGIF)
	
	command = ('\"'+ imagesx+'\"')
	subprocess.Popen(command, shell=True)
	
	command = ('reconknown\\'+ imagesxGIF)
	subprocess.Popen(command, shell=True)
...
def openImagedx():
	...
	global imagedx
	imagedx = tkinter.filedialog.asksaveasfilename(initialdir = ".",title = "Select file",filetypes = (("all files","*.jpg"),("all files","*.png"),("all files","*.jpeg")))
	print (imagedx)

	global imagedxGIF
	imagedxGIF = 'unknown.gif'
	print(imagedxGIF)
	
	command = ('convert  '+ imagedx + ' -resize 300x200 gif:'+'reconunknown\\'+imagedxGIF)
	subprocess.Popen(command, shell=True)
	print('reconunknown\\'+imagedxGIF)
	
	command = ('\"'+ imagedx+'\"')
	subprocess.Popen(command, shell=True)
	
	command = ('reconunknown\\'+ imagedxGIF)
	subprocess.Popen(command, shell=True)
...
def show_image():
	...
	img = PhotoImage(master = canvas1, file='reconunknown\\'+imagedxGIF)
...
def facerecognition():
		...
		os.chdir(os.path.expanduser('.'))
		slogan = tk.Canvas(root, width=300, height=200)
		img = PhotoImage(master = slogan, file='reconknown\\'+imagesxGIF)
		slogan.create_image(2, 2, anchor=NW, image=img)
		slogan.configure(bg="black")
		slogan.pack(side=LEFT, ipadx=2, ipady=3, pady=0, padx=5)
		slogan.place(x=10, y=150)
...
def face_detect_landmarks():
	...
	command = 'python recon68.py -i '+ imagedx +' -o '+'reconknown\\'+ imagedxREC
	subprocess.Popen(command, shell=True)
	time.sleep(2)
	...
	command = 'python recon68.py -i '+ imagesx + ' -o '+'reconunknown\\'+imagesxREC
	subprocess.Popen(command, shell=True)
...
def face_recognition_landmarks():
	...
	global imagedxREC
	imagedxREC = 'unknownRec.jpg'
	
	command = 'python recon68.py -i '+ imagedx +' -o '+'reconunknown\\'+imagedxREC
	subprocess.Popen(command, shell=True)
	time.sleep(2)
	
	global imagesxREC
	imagesxREC = 'knownRec.jpg'
		
	command = 'python recon68.py -i '+imagesx+' -o '+'reconknown\\'+imagesxREC
	subprocess.Popen(command, shell=True)
...
#   			('convert  '+ imagedx + ' -resize 300x200 gif:'+'reconunknown\\'+imagedxGIF)  
#	command = ('convert  '+'reconunknown\\'+imagedxREC + ' -resize 300x200 gif:'+'reconunknown\\'+imagedxGIFrec)
#	subprocess.Popen(command, shell=True)
	print(imagedxGIF)
	
#	command = ('reconunknown\\'+ imagedxREC)
#	subprocess.Popen(command, shell=True)
...
#	command = ('convert  '+'reconknown\\'+ imagesxREC + ' -resize 300x200 gif:'+'reconknown\\'+imagesxGIFrec)
#	subprocess.Popen(command, shell=True)
	print(imagesxGIF)
	
#	command = ('reconknown\\'+ imagesxREC)
#	subprocess.Popen(command, shell=True)
	...
	global sximage
	sximage = face_recognition.load_image_file(imagesx)
	print('KEY WAY POINTS - SXIMAGE')
	print(sximage)
	print('                             ')		
	# Find all the faces and face encodings in the image sx
	face_locationssx = face_recognition.face_locations(sximage)
	face_encodingssx = face_recognition.face_encodings(sximage, face_locationssx)
	print('FACE LOCATION KEY POINT SX 4')
	print(face_locationssx)
	print('                             ')	
	print('FACE ENCODINGS SX 128')
	print(face_encodingssx)
	print('                             ')		
	
	# Load  image dx
	dximage = face_recognition.load_image_file(imagedx)
	print('KEY WAY POINTS - DXIMAGE')
	print(sximage)
	print('                             ')		
	...
	face_locationsdx = face_recognition.face_locations(dximage)
	face_encodingsdx = face_recognition.face_encodings(dximage, face_locationsdx)[0]
	print('FACE LOCATION KEY POINT DX 4')
	print(face_locationsdx)
	print('                             ')	
	print('FACE ENCODINGS DX 128')
	print(face_encodingsdx)
	print('                             ')	
...
slogan = tk.Canvas(root, width=120, height=120)
img = PhotoImage(master = slogan, file="tsurugi.gif")
slogan.create_image(2, 2, anchor=NW, image=img)
slogan.configure(bg="black")
slogan.pack(side=LEFT, ipadx=2, ipady=3, pady=0, padx=5)
slogan.place(x=1210, y=460)
